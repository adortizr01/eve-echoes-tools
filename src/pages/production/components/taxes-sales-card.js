...
export default function TaxesBroker() {
  ...
  const {sales} = useTraderFee().getTradeModifier()
    ...
    <Card className={classes.root}>
      <CardContent>
        ...
        <Typography className={classes.title} gutterBottom>
          <NumberFormat
            ...
            value={ (currentShip.totalPrice * (1 + sales)) - currentShip.totalPrice}
            displayType={'text'}
            decimalScale={0}
      ...
      <Collapse in={expanded} timeout="auto" unmountOnExit>
        <CardContent>
          ...
          <NumberFormat
            className="w-30 bg-white px-2 text-right rounded shadow-inner"
            value={(currentShip.totalMineral * (1 + sales)) - currentShip.totalMineral}
            displayType={'text'}
            decimalScale={0}
            thousandSeparator={true}
            suffix={" ISK"}
          ...
          <NumberFormat
            className="w-30 bg-white px-2 text-right rounded shadow-inner"
            value={(currentShip.totalPlanetary * (1 + sales)) - currentShip.totalPlanetary}
            displayType={'text'}
            decimalScale={0}
            thousandSeparator={true}
            suffix={" ISK"}
